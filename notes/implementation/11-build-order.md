...
| 2 | §9.1 request shape; `M.infill` | `:Hive complete` still works; a FIM request round-trips against the local server |
...
| 6 | §8 prompt assembly + budget | `:Hive submit` fills the hole; trimming is reported; the §8.3.7 window is read from `/props` and a budget over it is refused |
